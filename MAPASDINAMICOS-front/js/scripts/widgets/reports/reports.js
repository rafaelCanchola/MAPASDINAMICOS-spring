          ...
          request : function(params,action,typeGraph){
                                ...
                                if (json){
                                    if (json.response.sucessfull){
                                        ...
                                        switch(action){
                                                  ...
                                                  case "getSeed":
                                                            setTimeout(function(){
                                                                      var source=connections.reports.downloadCvs;
                                                                      var r = $.extend(r, source);
                                                                      var path = r.url+'&filename=predios&id='+json.data+'&currentyear='+obj.options.data.userActive.currentyear;
                                                                      window.open(path);
                                                            },100);
                                                            break;
                                                  case "getSeedPdf":
                                                            setTimeout(function(){
                                                                      var source=connections.reports.downloadPdf;
                                                                      var r = $.extend(r, source);
                                                                      var path = r.url+'&filename=predios&id='+json.data+'&currentyear='+obj.options.data.userActive.currentyear;
                                                                      window.open(path);
                                                            },100);
                    ...
                    switch (action) {
                              ...
                              case 'getGraph':source=connections.reports.getGraph;
                                        ...
                                        //r.url=r.url+'&user='+obj.options.data.userActive.id+'&json='+JSON.stringify(params);
                                        break;
                              case 'getSeed':source=connections.reports.getSeed;
                                        r = $.extend(r, source);
                                        r.data={action:'get',user:obj.options.data.userActive.id,json:JSON.stringify(params),currentyear:obj.options.data.userActive.currentyear};
                                        //r.url=r.url+'&user='+obj.options.data.userActive.id+'&json='+JSON.stringify(params);
                                        break;
                              case 'getSeedPdf':source=connections.reports.getSeedPdf;
                                        r = $.extend(r, source);
                                        r.data={action:'get',user:obj.options.data.userActive.id,json:JSON.stringify(params),currentyear:obj.options.data.userActive.currentyear};
                                        //r.url=r.url+'&user='+obj.options.data.userActive.id+'&json='+JSON.stringify(params);
                                        break;
		...
		this.update();
          ...
          getSection4:function(){
                    ...
                    var chain='<div class="section4 section">'+
                                        obj.getBlocker()+
                                        '<div class="Header">'+
                                                  '<div class="title_report">Lista de predios</div>'+
                                                  '<div class="exportOptions">'+
                                                            ...
                                                            '<div class="itemReport replaceFolio"><div class="template_reports trep_replace" title="Remplazar predio" alt="Remplazar predio" ></div></div>'+
	  ...
	  buildStructure:function(){
          ...
          events:function(){
                    ...
                    */
                    ...
                    $("."+obj.id+" .trep_replace").click(function(){
                              obj.showBlockReplace();
                    });
          ...
          showReport:function(data){
                    ...
                    $("."+obj.id+" .title_report").html('Lista de predios');
                    ...
                    for(var x in data){
                              ...
                              var enableReplace = ((i.estatus.indexOf('Disponible')!=-1)&&((obj.options.data.userActive.roleId==1)||(obj.options.data.userActive.roleId==2)))?'replace="true"':'';
                              chain+='<div class="Row" '+enableReplace+' ref="'+i.wkt+'" feature="'+i.gid+'">';
                              for(var f in source){
          ...
          showBlockReplace:function(){
                    var obj = this;
                    var o = obj.options.data;
                    var title = 'Seleccione el folio a remplazar';
                    var item = $(".app_bottom_section_universe_replace");
                    item.show();
                    if(!obj.modalReplace){
                              item.reportsUniverse({data:{userActive:o.userActive,title:title,replace:true,itemToReplace:obj.itemToReplace}});
                              obj.modalReplace=true;
                    }else{
                              item.reportsUniverse('reload',obj.itemToReplace);
                    }
                    
          },
          showItemReplace:function(){
                    var obj = this;
                    $("."+obj.id+" .replaceFolio").show();
          },
          hideItemReplace:function(){
                    var obj = this;
                    $("."+obj.id+" .replaceFolio").hide();
