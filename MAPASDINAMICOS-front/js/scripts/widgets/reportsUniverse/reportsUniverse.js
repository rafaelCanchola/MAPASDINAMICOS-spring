define(["connections","map","validator","Alert","features","structure"], function(connections,map,validator,Alert,features,structure){
$.widget( "custom.reportsUniverse", {
          clase:'custom_reportsUniverse',
          countSpinner:0,
          fields:{},
          visible:false,
          counterFilter:0,
          ...
          options:{
                    data:{userActive:null,title:'',replace:false,itemToReplace:null},
                    controller:null
          },
          getFilter:function(){
                    var obj = this;
          ...
          request : function(params,action,typeGraph){
                                ...
                                if (json){
                                    if (json.response.sucessfull){
                                        ...
                                        switch(action){
                                                  ...
                                                  case "getSeed":
                                                            setTimeout(function(){
                                                                      var source=connections.reportsUniverse.downloadCvs;
                                                                      var r = $.extend(r, source);
                                                                      var path = r.url+'&filename=reporte&id='+json.data+'&currentyear='+obj.options.data.userActive.currentyear;
                                                                      window.open(path);
                                                            },100);
                                                            break;
                                                  case "getSeedPdf":
                                                            setTimeout(function(){
                                                                      var source=connections.reportsUniverse.downloadPdf;
                                                                      var r = $.extend(r, source);
                                                                      var path = r.url+'&filename=reporte&id='+json.data+'&currentyear='+obj.options.data.userActive.currentyear;
                                                                      window.open(path);
                                                            },100);
                                                  ...
                                                  case "replace":
                                                            ...
                                                            setTimeout(function(){
                                                                      structure.updateListPredios();
                                                                      $('body').notification({data:['El folio seleccionado ha sido remplazado']});
                                                            },500);
                                                            break;
                                                  case "getImages":
                                                            obj.buildImageSection(typeGraph,json.data);
                                                            break;
                                        }
                                    }else{
                    ...
                    switch (action) {
                              case 'getFields':source = connections.reportsUniverse.getFields;
                                        r = $.extend(r, source);
                                        r.data=params;
                                        break;
                              case 'getReport':source=connections.reportsUniverse.getReport;
                                        r = $.extend(r, source);
                                        r.data={action:'getall',user:obj.options.data.userActive.id,rows:obj.totalRows,page:obj.pagination.currentPage,currentyear:obj.options.data.userActive.currentyear,selected:obj.options.data.itemToReplace};
                                        if (params) {
                                                r.data.json = JSON.stringify(params);
                                        }
                                        //r.url=r.url+'&user='+obj.options.data.userActive.id+'&json='+JSON.stringify(params);
                                        break;
                              case 'getGraph':source=connections.reportsUniverse.getGraph;
                                        ...
                                        //r.url=r.url+'&user='+obj.options.data.userActive.id+'&json='+JSON.stringify(params);
                                        break;
                              case 'getSeed':source=connections.reportsUniverse.getSeed;
                                        r = $.extend(r, source);
                                        r.data={action:'get',user:obj.options.data.userActive.id,json:JSON.stringify(params),currentyear:obj.options.data.userActive.currentyear};
                                        //r.url=r.url+'&user='+obj.options.data.userActive.id+'&json='+JSON.stringify(params);
                                        break;
                              case 'getSeedPdf':source=connections.reportsUniverse.getSeedPdf;
                                        r = $.extend(r, source);
                                        r.data={action:'get',user:obj.options.data.userActive.id,json:JSON.stringify(params),currentyear:obj.options.data.userActive.currentyear};
                                        //r.url=r.url+'&user='+obj.options.data.userActive.id+'&json='+JSON.stringify(params);
                                        break;
                              case 'replace':
                                        source=connections.folios.replace;
                                        r = $.extend(r, source);
                                        r.data= $.extend({}, {/*user:obj.options.data.userActive.id,*/currentyear:obj.options.data.userActive.currentyear}, params);
                                        break;
                              case 'getImages':
                                        source=connections.reportsUniverse.getImages;
                                        r = $.extend(r, source);
                                        r.data= $.extend({}, {currentyear:obj.options.data.userActive.currentyear}, params);
                                        break;
                                       
                    }
                    $.ajax(r);
          },
          buildImageSection:function(id,data){
                    var obj = this;
                    var chain = '';
                    if ((data)&&(data.length>0)) {
                              var clase = 'doctsUniverse'+id;
                              var productor = $("."+obj.id+" #"+id).attr('prod');
                              var path='docs/'+productor+'/';
                              for(var x in data){
                                        var image = data[x];
                                        var label = (x==0)?'Ver':'';
                                        var idItem = (x==0)?clase:'';
                                        chain+='<a '+idItem+' class="'+clase+'" rel="'+clase+'" href="'+path+image+'" >'+label+'</a>';
                              }
                              $("."+obj.id+" #"+id).html(chain).unbind();
                              $("."+obj.id+" ."+clase).fancybox();
                              $("."+obj.id+" ."+clase).click();
                    }else{
                               chain = 'Sin documentos';
                               var folio = $("."+obj.id+" #"+id).attr('folio');
                               $("."+obj.id+" #"+id).html(chain).unbind().removeClass('enlace');
                               $('body').notification({data:['El folio '+folio+' no cuenta con documentos para visualizar']});
                    }
                    ...
                    var chain='<div class="section4 section">'+
                                        ...
                                        '<div class="Header">'+
                                                  ...
                                                  '<div class="exportOptions">'+
                                                            ...
                                                            '<div class="itemReport"><div class="template_reports trep_filter" title="Agregar filtro" alt="Agregar filtro" ></div></div>'+
                                                            '<div class="itemReport"><div class="template_reports trep_filter_clear" title="Limpiar filtro" alt="Limpiar filtro" ></div></div>'+
                                                            '<button class="button cancelReplace">Cancelar</button>'+
                                                            '<button class="button submitReplace">Remplazar</button>')+
	  ...
	  buildStructure:function(){
                    var obj=this;
                    obj.hide();
                    obj.id = this.element.attr('class');
                    var o = obj.options.data;
                    var chain = ''+
                              '<div class="'+obj.clase+'" align="center">'+
          ...
          events:function(){
                    ...
                    */
                    ...
                    $("."+obj.id+" .cancelReplace").click(function(){
                              obj.element.hide();
                    });
                    $("."+obj.id+" .submitReplace").click(function(){
                              var evento = function(description){
                                        var params = {selected:obj.options.data.itemToReplace,toreplace:obj.itemToReplace,description:description};
                                        obj.request(params,'replace');
                              }
                              if (obj.itemToReplace) {
                                        $("body").textArea({data:{label:'Descripci&oacute;n de remplazo',text:'',mode:'new',dataType:'alphanumeric',maxlength:200,event:evento}});
                              }else{
                                        $('body').notification({data:['Debe seleccionar un folio a remplazar']});
                              }
          ...
          showReport:function(data){
                    var obj=this;
                    var o = obj.options.data;
                    $("."+obj.id+" .title_report").html(obj.options.data.title);
                    ...
                    var header='<div class="Heading">';
                                        ...
                                        if ((o.userActive.roleId==6)&&(source[0].field!='documents')) {
                                            source.unshift({label:'Documentos',field:'documents'});
                                        }
                                        for(var x in source){
                    ...
                    for(var x in data){
                              ...
                              chain+='<div class="Row" ref="'+i.wkt+'" feature="'+i.gid+'">';
                              for(var f in source){
                                        var valor = (source[f].field=='documents')?'<p class="doctsUniverseLink enlace" id="'+i.gid+f+'" prod="'+i.folioprod+'" folio="'+i.foliopre+'" >Ver</p>':i[source[f].field];
                    ...
                    $("."+obj.id +" .doctsUniverseLink").each(function(){
                              $(this).click(function(){
                                        var enabled = $(this).attr('enabled');
                                        var id = $(this).attr('id');
                                        var folio = $(this).attr('folio');
                                        if (!enabled) {
                                            obj.request({folio:folio},'getImages',id);
                                        }
                              });
                    });
                    
          },
          reload:function(itemToReplace){
                    var obj = this;
                    obj.options.data.itemToReplace=itemToReplace;
                    obj.itemToReplace=null;
                    obj.firstReport=true;
                    obj.buildReport();
                    //features.updatePredios();
                    obj.clearFilter();
          ...
          showReportRows:function(data){
                    ...
                    var obj=this;
                    var chain='<div class="title_report">'+obj.options.data.title+'</div>';
                    chain+='<div class="results_table_filter">';
                    var source = obj.dataSend.fields;
